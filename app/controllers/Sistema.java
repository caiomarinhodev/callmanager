...
public class Sistema {
    ...
    public static List<Chamada> getListaDeChamadasGeraldoUsuario(Usuario u) {
        List<Chamada> li = dao.findByAttributeName(Chamada.class.getName(), "idOperador", String.valueOf(u.getId()));
        List<Chamada> lista = new ArrayList<>();
        Calendar calendar = Calendar.getInstance();
        java.util.Date data = calendar.getTime();
        Date date =  new Date(data.getYear(),data.getMonth()-1,30);
        for(Chamada c: li){
            //System.out.println("DIA DE COMPARAR: "+date);
            //System.out.println("DIA da chamada: "+c.getData());
            if(c.getData().after(date)){
                //System.out.println("DIA da chamada: "+c.getData());
               // System.out.println("é depois? : SIM ");
                lista.add(c);
            }else{
                //System.out.println("é depois? : NAO ");
            }
        }
        return lista;
    }
    ...
    public static void addChamada(String nome, String num1, String num2, String endereco, String cidade, String uf, String plano,
        ...
        if (addCliente(cliente)) {
            Chamada c = new Chamada(cliente, nota, horainicial, horafinal, status, operador, new Date(), dt, h);
            addChamada(c);
        }
    }
